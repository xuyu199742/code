...
class Tree
{
    ...
    public static function toLayer($lists = [], $pid = 0, $max_level = 0, $curr_level = 0)
    {
        ...
        foreach ($lists as $key => $value) {
            if ($value[self::$config['pid']] == $pid) {
                ...
                unset($lists[$key]);
                $children = self::toLayer($lists, $value[self::$config['id']], $max_level, $curr_level + 1);
                if (!empty($children)) {
                    $value[self::$config['children']] = $children;
                }
                $trees[] = $value;
            }
        }
     ...
     * @return array
     */
    public static function getchildrensId($lists = [], $pid = '')
    {
        $result = [];
        foreach ($lists as $value) {
            if ($value[self::$config['pid']] == $pid) {
                $result[] = $value[self::$config['id']];
                $result = array_merge($result, self::getchildrensId($lists, $value[self::$config['id']]));
            }
        }
        return $result;
    }
     ...
     * @return array
     */
    public static function getchildrens($lists = [], $pid = '')
    {
        $result = [];
        foreach ($lists as $value) {
            if ($value[self::$config['pid']] == $pid) {
                $result[] = $value;
                $result = array_merge($result, self::getchildrens($lists, $value[self::$config['id']]));
            }
        }
        return $result;
    }
