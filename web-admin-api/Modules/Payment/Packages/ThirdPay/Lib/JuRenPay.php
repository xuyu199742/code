...
class JuRenPay extends PayAbstract
{
    ...
    const APIS = [
        self::ALIPAY_QRCODE => [
            'JUREN_ALIPAY_QRCODE'  => self::NAME . '支付宝扫码',
        ],
        self::ALIPAY_H5 => [
            'JUREN_ALIPAY_H5'  => self::NAME . '支付宝H5',
    ...
    public function callback(): bool
    {
        try{
            ...
            if(!$sign){
                \Log::channel('sifang_pay_callback')->info('巨人支付回调错误提示：返回'.json_encode($callback_data));
                return false;
            }
            $key = $this->sdk_config['mch_key'] ?? '';
            $selfSign = strtoupper(NormalSignature::encrypt($callback_data,'sign','&key='.$key)); //生成签名
            if ($selfSign != $sign) {
                \Log::channel('sifang_pay_callback')->info('巨人支付回调错误提示：签名有误,内部签名：' . $selfSign . '返回数据：' . json_encode($original));
                return false;
            }
...
//	        list($head,$game_id,$order_id)=$visual_order_no_string;
            ...
            if(!$order){
                \Log::channel('sifang_pay_callback')->info('巨人支付回调错误提示：该订单未找到，请求IP：'.getIp());
                return false;
            }
            if($order->payment_status == PaymentOrder::SUCCESS){
                \Log::channel('sifang_pay_callback')->info('巨人支付回调成功提示：该订单已支付，请求IP：'.getIp());
                return true;
            }
            if ($order->amount == $money) {
                ...
                if ($order->thirdAddCoins()) {
                    \Log::channel('sifang_pay_callback')->info('巨人支付回调成功提示：用户金币充值成功');
                    return true;
                }else{
                    \Log::channel('sifang_pay_callback')->info('巨人支付回调错误提示：用户金币充值失败');
                    return false;
                }
            }
            \Log::channel('sifang_pay_callback')->info('巨人支付回调错误提示：用户金币充值失败，金额为'.$money);
            return false;
        }catch (\Exception $exception){
            \Log::channel('sifang_pay_callback')->info('巨人支付回调错误提示：系统异常'.$exception->getMessage());
            return false;
        }
    }
        ...
        try{
            ...
            switch ($order->provider->provider_key){
                ...
                case 'JUREN_WECHAT_H5':
                    $provider_key  = '8003';
                    break;
                case 'JUREN_ALIPAY_QRCODE':
                    $provider_key  = '8006';
                    break;
                case 'JUREN_ALIPAY_H5':
                    $provider_key  = '8007';
                    break;
            ...
            $key = $this->sdk_config['mch_key'] ?? '';
            //整理订单数据
	        $visual_order_no=array(
	        	'juRenPay',
		        $order->game_id,
		        $order->id
	        );
//	        $visual_order_no_string=implode('-', $visual_order_no);
            $data         = [
...
//            dd($data,$result);
            ...
            \Log::channel('sifang_pay_send')->info('巨人支付请求时间：'.$start_time.'响应时间：'.$end_time.'请求地址：'.$url.'请求数据：'.json_encode($data).'请求返回数据：'.$result['res']);
            //判断请求是否成功
            $success = $res['retCode'] ?? '';
