...
class YueFengPay extends PayAbstract
{
    const NAME = '悦风支付宝';//四方名称
    ...
    const APIS = [
        self::ALIPAY_QRCODE => [
            'zfb_qrcode_902' => self::NAME . '扫码',
        ],
        self::ALIPAY_H5 => [
            'zfb_h5_902' => self::NAME . 'H5',
        ],
    ];
    ...
    public function send(PaymentOrder &$order): bool
    {
        try{
            //需要加入签名的数据
            $data = [
                'pay_memberid'      => $this->sdk_config['zfb_memberid'] ?? '',//商户号
            ...
            //签名
            $data["pay_md5sign"] = NormalSignature::signature($data,'&key='.$this->sdk_config['zfb_key']);
            $data['pay_productname'] = '用户充值';
            //保存四方请求的数据
            $order->return_data = json_encode($data);
            return true;
        }catch (\Exception $exception){
            \Log::channel('sifang_pay_send')->info('悦风订单支付请求异常:'.$exception->getMessage());
            return false;
        }
    }
    ...
    {
        try{
            \Log::channel('sifang_pay_callback')->info('悦风订单回调时间:'.date('Y-m-d H:i:s',time()).'返回数据为：'.json_encode(request()->all()));
            //验证签名
            ...
            $sign                              = request('sign');
            //签名
            $selfsign = NormalSignature::signature($callback_data,'&key='.$this->sdk_config['zfb_key']);
            if ($sign != $selfsign) {
                \Log::channel('sifang_pay_callback')->info('悦风回调错误提示：签名有误，内部签名：'.$selfsign.'返回数据：'.json_encode($callback_data));
                return false;
            }
            //验证订单是否支付成功
            if ($callback_data['returncode'] != self::SUCCESS){
                \Log::channel('sifang_pay_callback')->info('悦风回调错误提示：订单支付未成功');
                return false;
            }
            //后台订单查询
            $order = PaymentOrder::where('order_no', $callback_data['orderid'])->where('payment_status', PaymentOrder::WAIT)->first();
            if (!$order){
                \Log::channel('sifang_pay_callback')->info('悦风回调错误提示：后台订单不存在');
                return false;
            }
            //验证充值金额和四方返回金额是否一致
            if ($order->amount != $callback_data['amount']){
                \Log::channel('sifang_pay_callback')->info('悦风回调错误提示：充值金额不符');
                return false;
            }
            //金币充值
            $order->payment_status = PaymentOrder::SUCCESS;
            $order->third_order_no = $callback_data['transaction_id'];//悦风返回的流水号
            $order->success_time   = $callback_data['datetime'];
            $order->callback_data   = json_encode($callback_data, true);
            if (!$order->thirdAddCoins()) {
                \Log::channel('sifang_pay_callback')->info('悦风回调错误提示：用户金币充值失败');
                return false;
            }
            return true;
        }catch (\Exception $exception){
            \Log::channel('sifang_pay_callback')->info('悦风回调错误提示：系统异常'.$exception->getMessage());
            return false;
        }
    }
    ...
    public function view(PaymentOrder $order): string
    {
        try{
            ...
            $url          = $this->sdk_config['zfb_gateway']; //拼装请求地址
            $start_time = date('Y-m-d H:i:s',time());//请求时间
            $result = $this->post($url, $data); //发送请求
            $str = '悦风支付请求时间：'.$start_time.'请求地址：'.$url.'请求数据：'.json_encode($data);
