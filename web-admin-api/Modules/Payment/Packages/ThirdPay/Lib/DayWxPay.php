...
class DayWxPay extends PayAbstract
{
    ...
    public function callback(): bool
    {
        try{
            ...
            if(!$sign){
                \Log::channel('sifang_pay_callback')->info('天天微信支付回调错误提示：返回'.json_encode($callback_data));
                return false;
            }
            $key = $this->sdk_config['mch_key'] ?? '';
            $selfSign = NormalSignature::encrypt($callback_data,'sign','&key='.$key); //生成签名
            if ($selfSign != $sign) {
                \Log::channel('sifang_pay_callback')->info('天天微信支付回调错误提示：签名有误,内部签名：' . $selfSign . '返回数据：' . json_encode($callback_data));
                return false;
            }
            ...
            if ($order && $order->amount == $money) {
                ...
                if ($order->thirdAddCoins()) {
                    \Log::channel('sifang_pay_callback')->info('天天微信支付回调成功提示：用户金币充值成功');
                    return true;
                }else{
                    \Log::channel('sifang_pay_callback')->info('天天微信支付回调错误提示：用户金币充值失败');
                    return false;
                }
            }
            return false;
        }catch (\Exception $exception){
            \Log::channel('sifang_pay_callback')->info('天天微信支付回调错误提示：系统异常'.$exception->getMessage());
            return false;
        }
    }
    ...
    public function send(PaymentOrder &$order): bool
    {
        ...
        \Log::channel('sifang_pay_send')->info('天天微信支付请求时间：'.$start_time.'响应时间：'.$end_time.'请求地址：'.$url.'请求数据：'.json_encode($data).'请求返回数据：'.json_encode($res,JSON_UNESCAPED_UNICODE));
        //判断请求是否成功
