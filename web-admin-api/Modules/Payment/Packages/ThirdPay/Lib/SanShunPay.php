...
class SanShunPay extends PayAbstract
{
	...
	const APIS = [
		self::WECHAT_QRCODE => [
			'SXP_WXSCAN' => self::NAME . '微信扫码',
		],
		self::WECHAT_H5 => [
			'SXP_WXH5'     => self::NAME . '微信H5',
		],
		self::ALIPAY_QRCODE => [
			'SXP_ALISCAN' => self::NAME . '支付宝扫码',
		],
		self::ALIPAY_H5 => [
			'SXP_ALIH5'     => self::NAME . '支付宝H5',
		]
	];
	...
	public function callback(): bool
	{
		try{
			...
			if(!$sign){
				\Log::channel('sifang_pay_callback')->info('三顺支付回调错误提示：返回'.json_encode($callback_data));
				return false;
			}
			...
			if ($selfSign != $sign) {
				\Log::channel('sifang_pay_callback')->info('三顺支付回调错误提示：签名有误,内部签名：' . $selfSign . '返回数据：' . json_encode($callback_data));
				return false;
			}
			...
			if ($order && $order->amount == $money && $callback_data['returncode'] == self::SUCCESS) {
				...
				if ($order->thirdAddCoins()) {
					\Log::channel('sifang_pay_callback')->info('三顺支付回调成功提示：用户金币充值成功');
					return true;
				}else{
					\Log::channel('sifang_pay_callback')->info('三顺支付回调错误提示：用户金币充值失败');
					return false;
				}
			}
			return false;
		}catch (\Exception $exception){
			\Log::channel('sifang_pay_callback')->info('三顺支付回调错误提示：系统异常'.$exception->getMessage());
			return false;
		}
	}
	...
	public function view(PaymentOrder $order): string
	{
		...
		$str = '三顺支付请求时间：'.$start_time.'请求地址：'.$url.'请求数据：'.json_encode($data);
		/*if ($result['res'] ?? ''){
		...
		//如果返回json，并提示错误
		return $this->show('SanShunPay', ['native'=>$data,'tjurl'=>$url]);
		//return $result['res'];
	}
